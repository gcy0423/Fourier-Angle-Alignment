...
from .modules import AdaptiveRotatedConv2d, RountingFunction
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if str(self.n_block) not in self.replace:
            ...
            self.conv2 = AdaptiveRotatedConv2d(
                in_channels=planes,
                out_channels=planes,
                kernel_size=3,
                stride=self.conv2_stride,
...
class ARCResNet(nn.Module):
    ...
    def __init__(self,
        ...
        if init_cfg is not None:
            if pretrained is not None:
                raise ValueError('init_cfg and pretrained should not be set at the same time.')

            if isinstance(init_cfg, dict) and init_cfg.get('type') == 'Pretrained':
                pretrained = init_cfg.get('checkpoint')
            else:
                raise TypeError(f'Unsupported init_cfg: {init_cfg}')
    ...
    def train(self, mode=True):
        ...
        freezed"""
        super(ARCResNet, self).train(mode)
        self._freeze_stages()
